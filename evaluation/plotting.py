...
def plot_all_metrics(source_dirs, output_dir, data_labels, group_labels):
    ...
    psnrs = np.array(data.psnrs for data in metric_sets).reshape((num_groups, len(metric_sets)/num_groups))
    ssims = np.array(data.ssims for data in metric_sets).reshape((num_groups, len(metric_sets)/num_groups))
    mses = np.array(data.mses for data in metric_sets).reshape((num_groups, len(metric_sets)/num_groups))
