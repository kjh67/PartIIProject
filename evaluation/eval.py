...
def eval_splats_cuda(project_dir, iterations, skip_rendering):
    ...
    for target in args.target_folders:
        if not args.skip_nerfs:
            if os.path.exists(os.path.join(target, "nerf")):
                eval_nerfs(target, args.iterations_to_evaluate, args.skip_rendering)
        if not args.skip_splats:
            if os.path.exists(os.path.join(target, "splat")):
                eval_splats_cuda(target, args.iterations_to_evaluate, args.skip_rendering)
